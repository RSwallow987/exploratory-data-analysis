...
# month
bymonth<-purchase_data%>%group_by(month,day)%>%count()
  
# 3. Turn this into a tsibble. What's the key, what's the index? Is it regular?
store_counts_ts <- as_tsibble(x = bymonth,
                          key = month,
                          index = day,
                          regular = TRUE)
...
store_counts_ts %>%
  ggplot(aes(x = day,
             y = n)) + geom_line(group = 1) +
  facet_wrap(~ month)

# 5. Do a seasonal decomposition using STL
sdecomp <- store_counts_ts%>%model(STL(n~trend()))%>%components()

autoplot(sdecomp) + xlab("Day")

#Why do the days only go up to 7 ? 
